...
// Journalise chaque changement de classement fait dans l'outil de qualification
// (swipe) dans un onglet dedie du Google Sheet (append-only). N'ecrit JAMAIS
// dans les donnees source : onglet separe cree automatiquement au premier usage.
// Best-effort : ne casse jamais l'action si la journalisation echoue.
// Prerequis : le service account doit avoir l'acces "Editeur" sur la feuille.
// NOTE : ce fichier est propre a l'outil de qualification. Le journal des
// assignations vit dans lib/sheet-log.ts et ne doit pas etre touche ici.
...
export async function logClassifChange(change: ClassifChange): Promise<{ ok: boolean; error?: string }> {
  try {
    ...
    const row = [
      ...
      change.ancienne || '',
      change.nouvelle || '',
      change.utilisateur || 'Equipe Fabiani-Naquet',
    ...
    console.error('[qualif-journal] echec journalisation:', e instanceof Error ? e.message : e);
